...
// For now disabled
    ...
    //内部就是讲roughness从0-1映射到0-6。因为最终默认要采样的模糊的cubemap有0-6个层级
    half mip = perceptualRoughnessToMipmapLevel(perceptualRoughness);
    half3 R = glossIn.reflUVW; //环境贴图uv坐标
    //使用 UNITY_SAMPLE_TEXCUBE_LOD(tex, R, mip)函数。这个就是我们之前说的不同粗糙度采样不同的mipmap的主要函数
    half4 rgbm = UNITY_SAMPLE_TEXCUBE_LOD(tex, R, mip);

    return DecodeHDR(rgbm, hdr); // HDR颜色解码
}
