...
//    2>直接对点积取绝对值（不完全准确，但效果可接受，省性能）
    ...
    */
    ...
    // Diffuse term  直接光照的漫反射
    half diffuseTerm = DisneyDiffuse(nv, nl, lh, perceptualRoughness) * nl;

    // Specular term 直接光照镜面反射部分
    ...
    float D = GGXTerm (nh, roughness); //法线分布函数计算
#else
    // Legacy
    // 旧版本保留而已，不会被用到，除非注释掉UnityStandardConfig.cginc中关于UNITY_BRDF_GGX的定义
    half V = SmithBeckmannVisibilityTerm (nl, nv, roughness);
    half D = NDFBlinnPhongNormalizedTerm (nh, PerceptualRoughnessToSpecPower(perceptualRoughness));
#endif
    //因为Diffuse项没有除Pi，所以这里乘Pi以保证比例相等（配平）。
...
#   endif
    ...
    /*
        specularTerm * light.color * FresnelTerm (specColor, lh) ==>  直接光照的镜面反射
        diffColor * light.color * diffuseTerm ==>   直接光照的漫反射

        diffColor * gi.diffuse ==> IBL漫反射部分
        surfaceReduction * gi.specular * FresnelLerp (specColor, grazingTerm, nv) ==> IBL镜面反射部分
    */
