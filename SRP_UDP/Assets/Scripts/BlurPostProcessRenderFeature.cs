...
[System.Serializable]
public class BlurPostProcessRenderFeatureSettings
{
...
public class BlurPostProcessRenderFeature : ScriptableRendererFeature
{
    ...
    public BlurPostProcessRenderFeatureSettings settings;

    private GrabPostProcessPassImpl m_grabPass;
    ...
    public override void Create()
    {
        ...
        m_grabPass = new GrabPostProcessPassImpl(m_BlurMaterial);
...
public class GrabPostProcessPassImpl : ScriptableRenderPass
{
    private Material m_BlurMaterial;
    ...
    public GrabPostProcessPassImpl(Material blurMateral)
    {
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        using (new ProfilingScope(cmd, profilingSampler))
        {
            ...
            var myBlur = stack.GetComponent<MyCustomePosterProcess>();
            if (myBlur != null && myBlur.IsActive())
            {
                float blurAmountX = myBlur.blurAmountX.value;
                float blurAmountY = myBlur.blurAmountY.value;
                //降低分辨率
                ...
                //横向纵向做Blur模糊
                cmd.SetGlobalVector("offsets", new Vector4(blurAmountX / Screen.width, 0, 0, 0));
                cmd.Blit(blurredID, blurredID2, m_BlurMaterial); //未指明pass，默认用第一个
                cmd.SetGlobalVector("offsets", new Vector4(0, blurAmountY / Screen.height, 0, 0));
                cmd.Blit(blurredID2, blurredID, m_BlurMaterial);
                cmd.SetGlobalVector("offsets", new Vector4(blurAmountY / Screen.width, 0, 0, 0));
