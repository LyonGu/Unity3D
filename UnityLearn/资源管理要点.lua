...
--[==[
    1 看下reskit那个开源库
    {
        ...
        ResourcesRes <-- Res
        AssetBundleRes <-- Res ==》 下载AB使用
        AssetRes <-- Res =》 AB里的asset
        NetImageRes <-- Res
    ...
    框架1： 1 每个loader有自己的list存储IRes对象 ResMgr有全局容器管理IRes对象  2 每个IRes对象有自己的加载状态 3 同步加载时放到一个等待列表里去加载
...
--[==[
    ...
    {
        引用计数管理
        同步加载接口, 异步加载接口 ==> 异步可以考虑使用 async和await  https://www.zhihu.com/question/334968458/answer/749437646
        加载进度
        加载完成回调
        支持不同类型加载
        回收资源接口
        ...
        设计下最小类型单元结构
        ab包的依赖项，可以打包的时候就存储好，这样就不用运行去查了

        一个开源框架 感觉不错https://gitee.com/0709/unity-asset-load-frame-work.git
    }
