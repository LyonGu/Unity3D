...
public class AnimMapBaker{
    ...
    {
        int curClipFrame = 0;
        float sampleTime = 0;  //记录采样时间
        float perFrameTime = 0;

        curClipFrame = Mathf.ClosestPowerOfTwo((int)(curAnim.clip.frameRate * curAnim.length)); //当前动作总帧数
        perFrameTime = curAnim.length / curClipFrame; ; //每帧多少时间
