...
public static class ProjectileMath
{
    ...
    public static Vector2[] ProjectileArcPoints(int iterations, float speed, float distance, float gravity, float angle)
    {
        ...
        {
            float x = iterationSize * i;  //水平位置
            float t = x / (speed * Mathf.Cos(radians)); // sx/(vx) ==> sx/(v * Mathf.Cos(radians)) 算出运行时间
            float y = -0.5f * gravity * (t * t) + speed * Mathf.Sin(radians) * t; //竖直位置
