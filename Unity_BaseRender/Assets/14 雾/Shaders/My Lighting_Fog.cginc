#if !defined(MY_LIGHTING_INCLUDED_Fog)
#define MY_LIGHTING_INCLUDED_Fog
...
float4 ApplyFog (float4 color, Interpolators i) {
	//color 为片段颜色
	#if FOG_ON
		//雾效果基于视距，该视距等于摄影机位置和片段的世界位置之间的矢量长度
		float viewDistance = length(_WorldSpaceCameraPos - i.worldPos.xyz);
		#if FOG_DEPTH
			viewDistance = UNITY_Z_0_FAR_FROM_CLIPSPACE(i.worldPos.w);
		#endif
		//雾密度函数 UNITY_CALC_FOG_FACTOR_RAW
		//该雾密度函数由UNITY_CALC_FOG_FACTOR_RAW宏计算得出。
		//这个宏创建unityFogFactor变量，可以使用它在雾色和片段颜色之间进行插值。雾的颜色存储在unity_FogColor中
