	...
	--[==[
		...
		音频资源内存优化
		{
			...
			LoadType
			{
				...
				Decompress On Load : 音频文件加载后就会解压缩，以【未压缩】的形式存在内存中，内存占用较大 ==》 适用频繁重复的短音频（如枪声）
				Compressed In Memory：音频文件以【压缩】的形式存在内存中，播放时解压  内存占用小一些 ==》 适用大部分音频
