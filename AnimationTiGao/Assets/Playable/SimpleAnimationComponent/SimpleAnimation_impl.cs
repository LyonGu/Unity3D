...
public partial class SimpleAnimation: MonoBehaviour, IAnimationClipSource
{
    ...
    private class StateEnumerable : IEnumerable<State>
    {
        ...
        class StateEnumerator : IEnumerator<State>
        {
            ...
            {
                m_Owner = owner;
                m_Impl = m_Owner.m_PlayableBehaviour.GetStates().GetEnumerator();
                Reset();
            }
    ...
    protected SimpleAnimationPlayable m_PlayableBehaviour;
    ...
    private void Initialize()
    {
        ...
        m_Graph = PlayableGraph.Create("SimpleAnimation");
        ...
        m_PlayableBehaviour = playable.GetBehaviour();
        m_PlayableBehaviour.onDone += OnPlayableDone;
        if (m_States == null)
        {
        ...
        if (m_States != null)
        {
            foreach (var state in m_States)
            {
                if (state.clip)
                {
                    m_PlayableBehaviour.AddClip(state.clip, state.name);
                }
            }
        }
        ...
        AnimationPlayableUtilities.Play(m_Animator, m_PlayableBehaviour.playable, m_Graph);
        Play();
        Kick();
        m_Initialized = true;
    }
    ...
    {
        if ( m_PlayableBehaviour != null && m_Clip != null && m_PlayableBehaviour.GetState(kDefaultStateName) == null )
        {
            m_PlayableBehaviour.AddClip(m_Clip, kDefaultStateName);
            Kick();
        }
    }
    ...
    //检测clip是否是Legacy类型，Legacy类型是使用Animation播放
    //Legacy类型的话会抛出异常
