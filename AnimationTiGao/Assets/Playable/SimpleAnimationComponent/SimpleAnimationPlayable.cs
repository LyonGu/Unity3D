...
public partial class SimpleAnimationPlayable : PlayableBehaviour
{
    ...
    private StateInfo DoAddClip(string name, AnimationClip clip)
    {
        ...
        //Find at which input the stateHandle will be connected
        ...
        //使用clip创建一个AnimationClipPlayable
    ...
    public bool AddClip(AnimationClip clip, string name)
    {
        ...
        if (state != null)
        {
            Debug.LogError(string.Format("Cannot add stateHandle with name {0}, because a stateHandle with that name already exists", name));
            return false;
        }
    ...
    public bool RemoveClip(string name)
    {
        ...
        if (state == null)
        {
            Debug.LogError(string.Format("Cannot remove stateHandle with name {0}, because a stateHandle with that name doesn't exist", name));
            return false;
        }
    ...
    public bool Play(string name)
    {
        ...
        if (state == null)
        {
            Debug.LogError(string.Format("Cannot play stateHandle with name {0} because there is no stateHandle with that name", name));
            return false;
        }
    ...
    public bool PlayQueued(string name, QueueMode queueMode)
    {
        ...
        if (state == null)
        {
            Debug.LogError(string.Format("Cannot queue Play to stateHandle with name {0} because there is no stateHandle with that name", name));
            return false;
        }
    ...
    public void Rewind(string name)
    {
        ...
        if (state == null)
        {
            Debug.LogError(string.Format("Cannot Rewind stateHandle with name {0} because there is no stateHandle with that name", name));
            return;
        }
    ...
    private void RemoveClones(StateInfo state)
    {
        ...
        while (it != null)
        {
            ...
            StateInfo queuedState = m_States[it.Value.stateHandle.index];
            if (queuedState.ParentStateHandle.index == state.index)
            {
    ...
    public bool Stop(string name)
    {
        ...
        if (state == null)
        {
            Debug.LogError(string.Format("Cannot stop stateHandle with name {0} because there is no stateHandle with that name", name));
            return false;
        }
    ...
    private bool IsClonePlaying(StateInfo state)
    {
        for (int i = 0; i < m_States.Count; i++)
        {
            ...
            if (otherState.isClone && otherState.enabled && otherState.ParentStateHandle.index == state.index)
            {
                return true;
            }
        }
    ...
    public bool Crossfade(string name, float time)
    {
        ...
        if (state == null)
        {
            Debug.LogError(string.Format("Cannot crossfade to stateHandle with name {0} because there is no stateHandle with that name", name));
            return false;
        }
    ...
    public bool CrossfadeQueued(string name, float time, QueueMode queueMode)
    {
        ...
        if (state == null)
        {
            Debug.LogError(string.Format("Cannot queue crossfade to stateHandle with name {0} because there is no stateHandle with that name", name));
            return false;
        }
    ...
    public bool Blend(string name, float targetWeight, float time)
    {
        ...
        if (state == null)
        {
            Debug.LogError(string.Format("Cannot blend stateHandle with name {0} because there is no stateHandle with that name", name));
            return false;
        }
    ...
    {
        StateInfo state = m_States[index];
        //把AnimationClipPlayable连接到m_Mixer上
        ...
        for (int i = 0; i < m_States.Count; i++)
        {
            ...
            if (state.enabledDirty)
            {
                ...
                if (!keepStoppedPlayablesConnected)
                {
                    ...
                    //if stateHandle is disabled but the corresponding input is connected, disconnect it
    ...
    private void ClearQueuedStates()
    {
        using (var it = m_StateQueue.GetEnumerator())
        {
            while (it.MoveNext())
            {
                ...
                m_States.StopState(queuedState.stateHandle.index, true);
            }
        }
        m_StateQueue.Clear();
    }
    ...
    private void UpdateQueuedStates()
    {
        ...
        while(it != null)
        {
            ...
            {
                Crossfade(queuedState.stateHandle.index, queuedState.fadeTime);
