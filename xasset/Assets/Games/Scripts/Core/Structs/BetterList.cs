...
//https://www.cnblogs.com/lanyelinxiang/p/17014546.html
/// System.Collections.Generic.List的改进版本，Clear()时不释放上的缓冲区，导致更好的性能和更少的垃圾收集。
/// PRO:当你添加和删除项目时，BetterList执行得比List快(尽管如果你从一开始删除会慢一些)。
/// CON: BetterList在排序列表时表现更差。如果操作涉及排序，则使用标准List
/// <summary>
...
public class BetterList<T>
{
    ...
    /// <summary> 通过将数组的大小重置为零来清除数组。注意，内存实际上并没有释放
    /// Clear the array by resetting its size to zero. Note that the memory is not actually released.
    /// </summary>
